...
(function () {
  ...
  if ("IntersectionObserver" in window) {
    var moreObserver = new IntersectionObserver(
  ...
  function set_treeview_height(el) {
    var height = "";
    if (!el.parents(".form").length && !el.parents("#menu").length) {
      var vh = "100vh";
  ...
  Sao.View.TreeXMLViewParser = Sao.class_(Sao.View.XMLViewParser, {
    ...
    _parse_field: function (node, attributes) {
      ...
      var ColumnFactory = Sao.View.TreeXMLViewParser.WIDGETS[attributes.widget];
      ...
      if ("symbol" in attributes) {
        column.suffixes.push(new Sao.View.Tree.Symbol(attributes, 1));
      }
      if (~["url", "email", "callto", "sip"].indexOf(attributes.widget)) {
        column.prefixes.push(new Sao.View.Tree.Affix(attributes, attributes.widget));
      }
      ...
      if ("symbol" in attributes) {
        column.prefixes.push(new Sao.View.Tree.Symbol(attributes, 0));
      }
      if (!this.view.attributes.sequence && !this.view.children_field && this.field_attrs[name].sortable !== false) {
    ...
    _parse_button: function (node, attributes) {
      var column = new Sao.View.Tree.ButtonColumn(this.view, attributes);
      this.view.columns.push(column);
    },
  });
  ...
  Sao.View.Tree = Sao.class_(Sao.View, {
    ...
    init: function (view_id, screen, xml, children_field) {
      ...
      this.columns = [];
      this.selection_mode = screen.attributes.selection_mode || Sao.common.SELECTION_MULTIPLE;
      this.el = jQuery("<div/>");
      this.scrollbar = jQuery("<div/>").appendTo(
        jQuery("<div/>", {
      ...
      if (this.children_field) {
        ...
        this.expander.on("click keypress", Sao.common.click_press(this.unfold.bind(this)));
        Sao.common.ICONFACTORY.get_icon_url("tryton-unfold-" + this.expander.action).then((url) => {
      ...
      for (const column of this.columns) {
        ...
        th.uniqueId();
        var label = jQuery("<label/>").text(column.attributes.string).attr("title", column.attributes.string);
        if (this.editable) {
      ...
      this.colgroup.prepend(
        jQuery("<col/>", {
          class: "tree-menu",
        })
      );
      this.thead.children().prepend(
        jQuery("<th/>", {
          class: "tree-menu",
        })
      );
    ...
    tree_menu: function (evt) {
      ...
      menu.append(
        jQuery("<li/>", {
          role: "presentation",
          class: this.selected_records.length || !navigator.clipboard ? "" : "disabled",
        }).append(
          jQuery("<a/>", {
            role: "menuitem",
            href: "#",
    ...
    save_optional: function (store = true) {
      ...
      if (store) {
        var tree_optional_model = new Sao.Model("ir.ui.view_tree_optional");
        tree_optional_model.execute("set_optional", [this.view_id, fields], {});
      }
    ...
    get editable() {
      return parseInt(this.attributes.editable || 0, 10) && !this.screen.attributes.readonly;
    },
    get creatable() {
      if (this.editable) {
    ...
    sort_model: function (e) {
      ...
      if (arrow.data("order") == "ASC") {
        ...
        Sao.common.ICONFACTORY.get_icon_url("tryton-arrow-up").then(function (url) {
          arrow.attr("src", url);
        });
        ...
        Sao.common.ICONFACTORY.get_icon_url("tryton-arrow-down").then(function (url) {
          arrow.attr("src", url);
        });
      ...
      if (!jQuery.isEmptyObject(unsaved_records) || this.screen.search_count == this.group.length || this.group.parent) {
        this.screen.search_filter(search_string, true).then((ids) => {
        ...
        this.columns.forEach((col) => {
          ...
          if (!record.is_loaded(col.attributes.name)) {
            try {
              ...
              Sao.Logger.error("Error loading " + `${this.attributes.name} for ${record}`);
    ...
    drag_data_get: function (evt) {
      ...
      var set_dragged_row = function (row) {
        if (row.el[0] === evt.target) {
          ...
          evt.originalEvent.dataTransfer.setData("position", row_position);
        }
    ...
    drag_data_received: function (evt) {
      ...
      if ((evt.ctrlKey || evt.metaKey) && this.children_field) {
        ...
        if (evt.shiftKey) {
          ...
          if (sibling_row) {
            ...
            dest_position = (parent_row || this).rows.indexOf(sibling_row) + 1;
          } else {
            parent_row = null;
            dest_position = 0;
          }
          ...
          if (sibling_row) {
            parent_row = sibling_row.parent_;
            dest_position = (parent_row || this).rows.indexOf(sibling_row);
          } else {
            parent_row = null;
      ...
      if (current_row) {
        ...
        var successor = jQuery(this.tbody.children()[original_position]);
      ...
      if (parent_row) {
        dest_group_prm = parent_row.record.children_group(this.children_field);
      } else {
      ...
      dest_group_prm.then((dest_group) => {
        ...
        if (origin_group === dest_group) {
          ...
          origin_group.record_removed.splice(origin_group.record_removed.indexOf(record));
        ...
        if (this.attributes.sequence) {
          row.record.group.set_sequence(this.attributes.sequence, this.screen.new_position);
        }
      });
    },
    get_fields: function () {
    ...
    display: function (selected, expanded) {
      ...
      if (jQuery.isEmptyObject(selected)) {
        ...
        if (this.selection.prop("checked") && !this.selection.prop("indeterminate")) {
          for (const record of this.screen.group.slice(this.rows.length, this.display_size)) {
      ...
      const group_records = (group, root) => {
        ...
        for (const record of group) {
          ...
          if (Sao.common.contains(expanded, path)) {
            const children = record.field_get_client(this.children_field);
            Array.prototype.push.apply(records, group_records(children, path));
          }
        }
        return records;
      };
      ...
      const row_records = (rows) => {
        ...
        for (const row of rows) {
          ...
          if (row.is_expanded()) {
            Array.prototype.push.apply(records, row_records(row.rows));
          }
        }
        return records;
      };
      var min_display_size = Math.min(this.group.length, this.display_size);
      if (this.children_field) {
        if (!Sao.common.compare(group_records(this.group.slice(0, min_display_size), []), row_records(this.rows))) {
          this.construct();
        }
      } else if (min_display_size > this.rows.length && Sao.common.compare(this.group.slice(0, this.rows.length), row_records(this.rows))) {
        this.construct(true);
      } else if (min_display_size != this.rows.length || !Sao.common.compare(this.group.slice(0, this.rows.length), row_records(this.rows))) {
        this.construct();
      }
      ...
      var min_width = [];
      var tree_column_optional = Sao.Screen.tree_column_optional[this.view_id] || {};
      for (const column of this.columns) {
        visible_columns += 1;
        ...
        if (tree_column_optional.hasOwnProperty(name)) {
          ...
          optional = Boolean(parseInt(column.attributes.optional || "0", 10));
        }
        var invisible = decoder.decode(column.attributes.tree_invisible || "0");
        ...
        if (!column.get_visible()) {
          ...
          !column.col.hasClass("draggable-handle") &&
      ...
      if (this.children_field) {
        this.columns.every((column) => {
          if (column.col.hasClass("draggable-handle") || column.header.hasClass("invisible")) {
            return true;
          } else {
      ...
      if (
        !this.table.hasClass("no-responsive") &
      ...
      return this.redraw(selected, expanded)
        .then(() => {
          ...
          if (this.display_size < this.group.length && !this.tbody.children().last().hasClass("more-row")) {
        ...
        .done(Sao.common.debounce(this.update_sum.bind(this), 250));
    },
    construct: function (extend) {
      ...
      // to minimize browser reflow
      for (const record of this.group.slice(this.rows.length, this.display_size)) {
        var RowBuilder;
        if (this.editable) {
      ...
      for (const [column, sum_widget] of this.sum_widgets) {
        ...
        for (i = 0; i < this.group.length; i++) {
          ...
          if (value !== null) {
            ...
            if (~records_ids.indexOf(record.id) || !selected_records) {
        ...
        if (loaded) {
          if (field.description.type == "timedelta") {
            ...
            selected_sum = Sao.common.timedelta.format(Sao.TimeDelta(null, selected_sum), converter);
            sum_ = Sao.common.timedelta.format(Sao.TimeDelta(null, sum_), converter);
          } else if (digit !== null) {
            var options = {};
            options.minimumFractionDigits = digit;
            options.maximumFractionDigits = digit;
            selected_sum = (selected_sum || 0).toLocaleString(Sao.i18n.BC47(Sao.i18n.getlang()), options);
            sum_ = (sum_ || 0).toLocaleString(Sao.i18n.BC47(Sao.i18n.getlang()), options);
          } else {
            selected_sum = (selected_sum || 0).toLocaleString(Sao.i18n.BC47(Sao.i18n.getlang()));
            sum_ = (sum_ || 0).toLocaleString(Sao.i18n.BC47(Sao.i18n.getlang()));
          }
          aggregate = selected_sum + "\n" + sum_;
        }
        sum_value.text(aggregate);
        sum_value.parent().attr("title", sum_label.text() + "\n" + sum_value.text());
      }
    },
    ...
    get listed_records() {
      ...
      const get_listed_records = (start) => {
        ...
        for (var idx = 0, len = this.n_children(row); idx < len; idx++) {
    ...
    update_selection: function () {
      ...
      } else if (selected_records.length == this.tbody.children().length && this.display_size >= this.group.length) {
    ...
    get_expanded_paths: function (starting_path = [], starting_id_path = []) {
      ...
      for (var path_idx = 0, len = this.n_children(row); path_idx < len; path_idx++) {
        ...
        if (row && row.is_expanded()) {
          ...
          id_paths = id_paths.concat(this.get_expanded_paths(path, id_path));
        }
      }
      return id_paths;
    },
    ...
    set_cursor: function (new_, reset_view) {
      ...
      if (path.length > 1) {
        prm = this.rows[path[0]].expand_to_path(path.slice(1), [
    ...
    save_row: function () {
      var i,
        prm,
        edited_row = this.edited_row;
      if (!this.editable || !this.edited_row) {
        return jQuery.when();
      }
      if (!this.edited_row.record.validate(this.get_fields(), false, false, true)) {
        var focused = false;
  ...
  Sao.View.Tree.Row = Sao.class_(Object, {
    ...
    _construct: function () {
      ...
      const on_click = (event_) => {
        if (this.expander && !this.is_expanded() && this.tree.n_children(this) <= Sao.config.limit) {
          this.toggle_row();
        }
      ...
      if (this.children_field) {
        ...
        this.expander.on("click keypress", Sao.common.click_press(this.toggle_row.bind(this)));
    ...
    redraw: function (selected, expanded) {
      ...
      if (this.children_field) {
        ...
        const update_expander = () => {
          var length = this.record.field_get_client(this.children_field).length;
          if (length && (this.is_expanded() || Sao.common.contains(expanded, row_id_path))) {
            this.expander.css("visibility", "visible");
            this.tree.expanded.add(this);
            this.expand_children(selected, expanded);
            this.update_expander(true);
          } else {
            this.expander.css("visibility", length ? "visible" : "hidden");
            this.update_expander(false);
          }
        };
        if (!this.record.is_loaded(this.children_field)) {
          this.record.load(this.children_field, true, false).done(update_expander);
        } else {
          update_expander();
        }
      }
    ...
    update_visible: function () {
      ...
      for (var i = 0; i < this.tree.columns.length; i++) {
        var column = this.tree.columns[i];
        var td = this._get_column_td(i);
        if ((column.header.is(":hidden") && thead_visible) || column.header.css("display") == "none") {
          td.hide();
    ...
    expand_children: function (selected, expanded) {
      return this.record.load(this.children_field, true, false).done(() => {
        if (this.rows.length === 0) {
          var children = this.record.field_get_client(this.children_field);
          children.forEach((record, pos, group) => {
            // The rows are added to the tbody after being rendered
            // to minimize browser reflow
            this.rows.push(new this.Class(this.tree, record, pos, this));
          });
        }
    ...
    select_row: function (event_) {
      if (this.tree.selection_mode == Sao.common.SELECTION_NONE) {
        ...
        var current_record;
        if (event_.shiftKey && this.tree.selection_mode != Sao.common.SELECTION_SINGLE) {
          current_record = this.tree.screen.current_record;
          this.tree.select_records(current_record, this.record);
        } else {
          if (!(event_.ctrlKey || event_.metaKey) || this.tree.selection_mode == Sao.common.SELECTION_SINGLE) {
    ...
    selection_changed: function () {
      ...
      if (is_selected) {
        ...
        this.tree.select_changed(this.tree.selected_records[0] || null);
      }
      this.tree.update_selection();
    },
    expand_to_path: function (path, selected) {
      if (path.length && this.record.field_get_client(this.children_field).length) {
        this.expander.css("visibility", "visible");
        this.tree.expanded.add(this);
        this.update_expander(true);
    ...
    next_column: function (path, editable, sign) {
      ...
      for (i = 0; i < this.tree.columns.length; i++) {
        column_index = (path + sign * (i + 1)) % this.tree.columns.length;
        ...
        if (editable) {
          var EditableBuilder = Sao.View.EditableTree.WIDGETS[column.attributes.widget];
          readonly = column.attributes.readonly || state_attrs.readonly || !EditableBuilder;
        } else {
          readonly = false;
        }
  ...
  Sao.View.Tree.RowEditable = Sao.class_(Sao.View.Tree.Row, {
    init: function (tree, record, pos, parent) {
      Sao.View.Tree.RowEditable._super.init.call(this, tree, record, pos, parent);
      this.edited_column = null;
      this.el.on("keypress", (event_) => {
        if (event_.which == Sao.common.RETURN_KEYCODE && this.tree.edited_row != this) {
    ...
    redraw: function (selected, expanded) {
      ...
      Sao.View.Tree.RowEditable._super.redraw.call(this, selected, expanded);
      ...
      for (i = 0; i < this.tree.columns.length; i++) {
        ...
        if (widget) {
          ...
          if (!this.record.is_loaded(column.attributes.name)) {
            this.record.load(column.attributes.name, true, false).done(callback);
          } else {
            callback();
          }
        }
    ...
    select_row: function (event_) {
      var body, listener;
      event_.stopPropagation();
      if (this.tree.edited_row && event_.currentTarget == this.tree.edited_row.el[0]) {
        return;
      }

      var current_record = this.tree.screen.current_record;
      if (this.record != current_record && current_record && !current_record.validate(this.tree.get_fields(), false, false, true)) {
        return;
      }
      ...
      const handler = (event_) => {
        if (event_.currentTarget == body[0] && body.hasClass("modal-open")) {
          return;
        }
    ...
    unset_editable: function () {
      this.tree.columns.forEach((col, idx) => {
        ...
        this.get_editable_el(td).empty().data("widget", null).hide().parents(".treeview td").addBack().removeClass("edited");
      });
    },
    set_editable: function () {
      ...
      for (var i = 0, len = this.tree.columns.length; i < len; i++) {
        ...
        var EditableBuilder = Sao.View.EditableTree.WIDGETS[col.attributes.widget];
        if (!col.attributes.readonly && EditableBuilder) {
          var widget = new EditableBuilder(this.tree, col.attributes);
          ...
          static_el.hide();
          editable_el.show();
          editable_el.parents(".treeview td").addBack().addClass("edited");
    ...
    key_press: function (event_) {
      ...
      if (
        (event_.which != Sao.common.TAB_KEYCODE &&
        ...
        jQuery(event_.currentTarget).find(".dropdown-menu:visible").length
      ) {
        return;
      }
      event_.preventDefault();
      ...
      if (column.field.validate(this.record)) {
        ...
        } else if (event_.which == Sao.common.UP_KEYCODE || event_.which == Sao.common.DOWN_KEYCODE || event_.which == Sao.common.RETURN_KEYCODE) {
          next_column = this.edited_column;
          this.record.validate(this.tree.get_fields()).then((validate) => {
            if (!validate) {
              var invalid_fields = this.record.invalid_fields();
              for (i = 0; i < this.tree.columns.length; i++) {
                var col = this.tree.columns[i];
                if (col.attributes.name in invalid_fields) {
                  next_column = i;
              ...
              this._get_column_td(next_column).find(":input,[tabindex=0]").focus();
            } else {
              var prm = jQuery.when();
              ...
              if (
                !next_row.length &&
                (event_.which == Sao.common.RETURN_KEYCODE ||
                  (event_.which == Sao.common.UP_KEYCODE && this.tree.screen.new_position == 0) ||
                  (event_.which == Sao.common.DOWN_KEYCODE && this.tree.screen.new_position == -1))
              ) {
                var model = this.tree.screen.group;
                var access = Sao.common.MODELACCESS.get(this.tree.screen.model_name);
                var limit = this.tree.screen.size_limit !== null && model.length >= this.tree.screen.size_limit;
                if (this.tree.creatable && access.create && !limit) {
                  prm
                    .then(() => this.tree.screen.new_())
                    .then((record) => {
                      var sequence = this.tree.attributes.sequence;
                      if (sequence) {
                        record.group.set_sequence(sequence, this.tree.screen.new_position);
                      }
                    });
                }
              } else {
                prm.then(() => {
                  this._get_column_td(next_column, next_row).trigger("click").find(":input,[tabindex=0]").focus();
                });
              }
            }
          });
          ...
          if (this.attributes.icon_type == "url") {
            if (value) {
              if (this.attributes.url_size) {
                ...
                url.searchParams.set(this.attributes.url_size, 20);
                value = url.href;
              }
  ...
  Sao.View.Tree.CharColumn = Sao.class_(Object, {
    ...
    render: function (record, cell) {
      ...
      if (!record.is_loaded(this.attributes.name)) {
        record.load(this.attributes.name, true, false).done(render).fail(render_error);
      } else {
        if (record.exception) {
          render_error();
        } else {
  ...
  Sao.View.Tree.BooleanColumn = Sao.class_(Sao.View.Tree.CharColumn, {
    ...
    render: function (record, cell) {
      var new_cell = !cell;
      cell = Sao.View.Tree.BooleanColumn._super.render.call(this, record, cell);
      var disabled = true;
      if (this.tree.editable) {
        if (new_cell) {
          cell.on("click", null, { record: record, cell: cell }, this.clicked.bind(this));
        }
        var state_attrs = this.field.get_state_attrs(record);
    ...
    clicked: function (evt) {
      ...
      if (!current_record || current_record.validate(fields, false, false, true)) {
  ...
  Sao.View.Tree.SelectionColumn = Sao.class_(Sao.View.Tree.CharColumn, {
    ...
    update_selection: function (record, callback) {
      ...
      Sao.common.selection_mixin.update_selection.call(this, record, this.field, callback);
    },
    get_textual_value: function (record) {
    ...
    update_text: function (cell, record) {
      if (!this.tree.editable && this.field.name + ":string" in record._values) {
        ...
        this.update_selection(record, () => {
          ...
          if (!found) {
            prm = Sao.common.selection_mixin.get_inactive_selection.call(this, value).then(function (inactive) {
              return inactive[1];
            });
          } else {
  ...
  Sao.View.Tree.MultiSelectionColumn = Sao.class_(Sao.View.Tree.CharColumn, {
    ...
    init: function (model, attributes) {
      Sao.View.Tree.MultiSelectionColumn._super.init.call(this, model, attributes);
      Sao.common.selection_mixin.init.call(this);
      this.init_selection();
    },
    init_selection: function (key) {
    ...
    update_text: function (cell, record) {
      if (!this.tree.editable && this.field_name + ":string" in record._values) {
        var text_value = this.get_textual_value(record);
        cell.text(text_value).attr("title", text_value);
      } else {
        this.update_selection(record, () => {
  ...
  Sao.View.Tree.ReferenceColumn = Sao.class_(Sao.View.Tree.CharColumn, {
    ...
    init: function (model, attributes) {
      Sao.View.Tree.ReferenceColumn._super.init.call(this, model, attributes);
      Sao.common.selection_mixin.init.call(this);
      this.init_selection();
    },
    init_selection: function (key) {
    ...
    get_cell: function () {
      var cell = Sao.View.Tree.ReferenceColumn._super.get_cell.call(this);
      cell.append(
        jQuery("<a/>", {
          href: "#",
        })
    ...
    get_textual_value: function (record) {
      var value = Sao.View.Tree.ReferenceColumn._super.get_textual_value.call(this, record);
      var model, name, text;
      if (!value) {
        model = "";
        name = "";
  ...
  Sao.View.Tree.TimeColumn = Sao.class_(Sao.View.Tree.CharColumn, {
    ...
    get_textual_value: function (record) {
      var value = this.field.get_client(record);
      return Sao.common.format_time(this.field.time_format(record), value);
    },
  });
  ...
  Sao.View.Tree.URLColumn = Sao.class_(Sao.View.Tree.CharColumn, {
    class_: "column-url",
    render: function (record, cell) {
      cell = Sao.View.Tree.URLColumn._super.render.call(this, record, cell);
      this.field.set_state(record);
      var state_attrs = this.field.get_state_attrs(record);
  ...
  Sao.View.Tree.ButtonColumn = Sao.class_(Object, {
    ...
    render: function (record, el) {
      ...
      var fields = jQuery.map(this.view.screen.model.fields, function (field, name) {
  ...
  Sao.View.EditableTree.Char = Sao.class_(Sao.View.Form.Char, {
    ...
    init: function (view, attributes) {
      Sao.View.EditableTree.Char._super.init.call(this, view, attributes);
  ...
  Sao.View.EditableTree.Date = Sao.class_(Sao.View.Form.Date, {
    ...
    init: function (view, attributes) {
      Sao.View.EditableTree.Date._super.init.call(this, view, attributes);
      Sao.View.EditableTree.editable_mixin(this);
    },
  });
  ...
  Sao.View.EditableTree.Time = Sao.class_(Sao.View.Form.Time, {
    ...
    init: function (view, attributes) {
      Sao.View.EditableTree.Time._super.init.call(this, view, attributes);
      Sao.View.EditableTree.editable_mixin(this);
    },
  });
  ...
  Sao.View.EditableTree.TimeDelta = Sao.class_(Sao.View.Form.TimeDelta, {
    ...
    init: function (view, attributes) {
      Sao.View.EditableTree.TimeDelta._super.init.call(this, view, attributes);
      Sao.View.EditableTree.editable_mixin(this);
    },
  });
  ...
  Sao.View.EditableTree.Integer = Sao.class_(Sao.View.Form.Integer, {
    ...
    init: function (view, attributes) {
      ...
      Sao.View.EditableTree.Integer._super.init.call(this, view, attributes);
      Sao.View.EditableTree.editable_mixin(this);
    },
    get width() {
      return;
  ...
  Sao.View.EditableTree.Float = Sao.class_(Sao.View.Form.Float, {
    ...
    init: function (view, attributes) {
      attributes = jQuery.extend({}, attributes);
      delete attributes.symbol;
      Sao.View.EditableTree.Float._super.init.call(this, view, attributes);
      Sao.View.EditableTree.editable_mixin(this);
    },
    get width() {
      return;
  ...
  Sao.View.EditableTree.Selection = Sao.class_(Sao.View.Form.Selection, {
    ...
    init: function (view, attributes) {
      Sao.View.EditableTree.Selection._super.init.call(this, view, attributes);
      Sao.View.EditableTree.editable_mixin(this);
    },
  });
  ...
  Sao.View.EditableTree.Boolean = Sao.class_(Sao.View.Form.Boolean, {
    ...
    init: function (view, attributes) {
      Sao.View.EditableTree.Boolean._super.init.call(this, view, attributes);
      Sao.View.EditableTree.editable_mixin(this);
    },
  });
  ...
  Sao.View.EditableTree.Many2One = Sao.class_(Sao.View.Form.Many2One, {
    ...
    init: function (view, attributes) {
      Sao.View.EditableTree.Many2One._super.init.call(this, view, attributes);
    },
  });
  ...
  Sao.View.EditableTree.Reference = Sao.class_(Sao.View.Form.Reference, {
    ...
    init: function (view, attributes) {
      Sao.View.EditableTree.Reference._super.init.call(this, view, attributes);
    },
  });
  ...
  Sao.View.EditableTree.One2One = Sao.class_(Sao.View.Form.One2One, {
    ...
    init: function (view, attributes) {
      Sao.View.EditableTree.One2One._super.init.call(this, view, attributes);
    },
  });
  ...
  Sao.View.EditableTree.One2Many = Sao.class_(Sao.View.EditableTree.Char, {
    ...
    init: function (view, attributes) {
      Sao.View.EditableTree.One2Many._super.init.call(this, view, attributes);
    },
  ...
  Sao.View.EditableTree.Binary = Sao.class_(Sao.View.Form.Binary, {
    ...
    init: function (view, attributes) {
      Sao.View.EditableTree.Binary._super.init.call(this, view, attributes);
      Sao.View.EditableTree.editable_mixin(this);
    },
  });
