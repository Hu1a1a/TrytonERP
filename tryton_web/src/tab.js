...
(function () {
  ...
  Sao.Tab = Sao.class_(Object, {
    ...
    menu_def: function () {
      return [
        ...
        {
          id: this.screen && Sao.common.MODELHISTORY.contains(this.screen.model_name) ? "revision" : null,
          icon: "tryton-history",
    ...
    create_toolbar: function () {
      ...
      const add_button = (item) => {
        ...
        var button = jQuery("<button/>", attributes).append(
          Sao.common.ICONFACTORY.get_icon_img(item.icon, {
            "aria-hidden": "true",
          })
        );
        ...
        this.buttons[item.id].click(item, (event) => {
          ...
          (this[item.id](this) || jQuery.when()).always(function () {
      ...
      toolbar
        .find(".btn-toolbar > .btn-group")
        .last()
        .addClass("hidden-xs")
        .find(".dropdown")
        .on("show.bs.dropdown", function () {
          jQuery(this).parents(".btn-group").removeClass("hidden-xs");
        })
        .on("hide.bs.dropdown", function () {
          jQuery(this).parents(".btn-group").addClass("hidden-xs");
        });
      return toolbar;
    },
    show: function () {
  ...
  Sao.Tab.tabs.close = function (warning) {
    if (warning && Sao.Tab.tabs.length) {
      return Sao.common.sur
        .run(Sao.i18n.gettext("The following action requires to close all tabs.\n" + "Do you want to continue?"))
        .then(function () {
  ...
  Sao.Tab.Form = Sao.class_(Sao.Tab, {
    ...
    init: function (model_name, attributes) {
      ...
      this.view_prm = this.screen.switch_view().done(() => {
        ...
        if (attributes.res_id) {
          ...
          screen.current_record = screen.group.get(attributes.res_id[0]);
          screen.display();
        } else {
          if (screen.current_view.view_type == "form") {
            screen.new_();
          }
          if (~["tree", "graph", "calendar"].indexOf(screen.current_view.view_type)) {
    ...
    create_toolbar: function () {
      ...
      var toolbars = screen.model.execute("view_toolbar_get", [], screen.context, false);
      [
        ["action", "tryton-launch", Sao.i18n.gettext("Launch action")],
        ["relate", "tryton-link", Sao.i18n.gettext("Open related records")],
        ["print", "tryton-print", Sao.i18n.gettext("Print report")],
      ].forEach((menu_action) => {
        var dropdown = jQuery("<div/>", {
          class: "btn-group dropdown",
          role: "group",
          ...
          .append(
            jQuery("<button/>", {
              ...
              .append(
                Sao.common.ICONFACTORY.get_icon_img(menu_action[1], {
                  "aria-hidden": "true",
                })
              )
              .append(
        ...
        button.click(function () {
          menu.find(["." + menu_action[0] + "_button", ".divider-button", "." + menu_action[0] + "_plugin", ".divider-plugin"].join(",")).remove();
          var buttons = screen.get_buttons().filter(function (button) {
            return menu_action[0] == (button.attributes.keyword || "action");
          });
          if (buttons.length) {
            menu.append(
              jQuery("<li/>", {
          ...
          for (const plugin of Sao.Plugins) {
            for (const spec of plugin.get_plugins(screen.model.name)) {
              var name = spec[0],
                func = spec[1],
          ...
          kw_plugins.forEach(function (plugin) {
            ...
            jQuery("<li/>", {
              ...
              .click(function (evt) {
                evt.preventDefault();
                var ids = screen.current_view.selected_records.map(function (record) {
                  return record.id;
                });
                var id = screen.current_record ? screen.current_record.id : null;
                var model_context = screen.context_screen ? screen.context_screen.model_name : null;
                func({
          ...
          var item = jQuery("<li/>", {
            ...
            .click((evt) => {
              ...
              this.modified_save().then(function () {
                ...
                Sao.Action.execute(exec_action, data, jQuery.extend({}, screen.local_context));
              });
            })
            .appendTo(menu);
        });
        ...
        if (menu_action[0] != "action") {
          button._can_be_sensitive = Boolean(menu.children().length);
        }

        if (menu_action[0] == "print" && toolbars.exports.length) {
      ...
      this.buttons.attach.on("dragover", false).on("drop", this.attach_drop.bind(this));
      return toolbar;
    },
    create_tabcontent: function () {
    ...
    compare: function (attributes) {
      ...
      return (
        ...
        compare(this.attributes.view_ids || [], attributes.view_ids || []) &&
        (attributes.view_ids || compare(this.attributes.mode || ["tree", "form"], attributes.mode || ["tree", "form"])) &&
        JSON.stringify(this.screen.local_context) === JSON.stringify(attributes.context) &&
        compare(this.attributes.search_value || [], attributes.search_value || []) &&
        JSON.stringify(this.screen.attributes.tab_domain) === JSON.stringify(attributes.tab_domain)
      );
    },
    _close_allowed: function () {
    ...
    modified_save: function () {
      ...
      if (this.screen.modified()) {
        return Sao.common.sur_3b.run(Sao.i18n.gettext("This record has been modified\n" + "do you want to save it?")).then((result) => {
          switch (result) {
            case "ok":
              return this.save();
            case "ko":
              ...
              return this.reload(false).then(() => {
                if (record_id < 0) {
                  ...
                  if (record_id != this.screen.current_record.id) {
                    return jQuery.Deferred().reject();
                  }
                }
              });
    ...
    save: function (tab) {
      ...
      return this.screen.save_current().then(
        () => {
          this.info_bar.add(Sao.i18n.gettext("Record saved."), "info");
        ...
        return this.screen
          ...
          .then(() => {
            ...
            if (this.screen.current_view.view_type != "form") {
              return this.screen.search_filter(this.screen.screen_container.search_entry.val()).then(() => {
    ...
    copy: function () {
      ...
      return this.modified_save().then(() => {
        return this.screen.copy().then(() => {
          this.info_bar.add(Sao.i18n.gettext("Working now on the duplicated record(s)."), "info");
          this.screen.count_tab_domain(true);
        });
      });
    },
    delete_: function () {
      ...
      return Sao.common.sur.run(msg).then(() => {
        return this.screen.remove(true, false, true).then(
          () => {
            this.info_bar.add(Sao.i18n.gettext("Records removed."), "info");
            this.screen.count_tab_domain(true);
          },
          () => {
            this.info_bar.add(Sao.i18n.gettext("Records not removed."), "danger");
          }
        );
      });
    },
    ...
    logs: function () {
      ...
      if (!record || record.id < 0) {
        this.info_bar.add(Sao.i18n.gettext("You have to select one record."), "info");
        return jQuery.when();
      }
    ...
    revision: function () {
      ...
      const set_revision = (revisions) => {
        return (revision) => {
          ...
          if (this.screen.current_view.view_type == "form" && revision && revision < revisions[revisions.length - 1][0]) {
      ...
      return this.modified_save().then(() => {
        var ids = this.screen.current_view.selected_records.map((record) => record.id);
        return this.screen.model.execute("history_revisions", [ids], this.screen.context).then((revisions) => {
          const revision = this.screen.context._datetime;
          if (revision) {
            // Remove a millisecond as microseconds are truncated
            revision.add(-1, "milliseconds");
          }
          new Sao.Window.Revision(revisions, revision, set_revision(revisions));
        });
      });
    },
    update_revision: function () {
      ...
      var label, title;
      if (revision) {
        var date_format = Sao.common.date_format(this.screen.context.date_format);
        var time_format = "%H:%M:%S.%f";
        var revision_label = " @ " + Sao.common.format_datetime(date_format + " " + time_format, revision);
        label = Sao.common.ellipsize(this.name, 80 - revision_label.length) + revision_label;
    ...
    set_buttons_sensitive: function () {
      ...
      if (!revision) {
        ...
        const accesses = new Map([
          ...
          ["save", (access.create || access.write) && modified && !this.screen.readonly],
        ...
        for (const [name, access] of accesses) {
          ...
          if (this.menu_buttons[name]) {
            this.menu_buttons[name].toggleClass("disabled", !access);
          }
        }
      } else {
        for (const name of ["new_", "save", "delete_", "copy", "import", "action"]) {
          if (this.buttons[name]) {
    ...
    attach: function (evt) {
      ...
      const preview = () => {
        if (this.attachment_preview.children().length) {
          ...
          this.attachment_preview.append(this._attachment_preview_el());
            ...
            jQuery("<li/>", {
              ...
              .append(
                jQuery("<input/>", {
                  ...
                  Sao.common.get_input_data(jQuery(this), function (data, filename) {
    ...
    attach_drop: function (evt) {
      ...
      jQuery.when.apply(jQuery, uris).then(function () {
        ...
        for (const argument of arguments) {
          argument.split("\r\n").filter(empty).forEach(window_.add_uri, window_);
        }
      });
    ...
    refresh_attachment_preview: function (force) {
      ...
      if (!Sao.common.compare(this.attachment_screen.domain, domain) || force) {
    ...
    email: function () {
      ...
      if (!this.buttons.email.prop("disabled")) {
        this.modified_save().then(() => {
          ...
          this.screen.model.execute("view_toolbar_get", [], this.screen.context).then(function (toolbars) {
            ...
            record.rec_name().then(function (rec_name) {
              function email(template) {
                new Sao.Window.Email(title + ": " + rec_name, record, prints, template);
              }
              Sao.common.selection(Sao.i18n.gettext("Template"), emails, true).then(email, email);
            });
          });
        });
      }
    ...
    refresh_resources: function (reload) {
      var record = this.screen.current_record;
      if (record) {
        record.get_resources(reload).always(this.update_resources.bind(this));
      } else {
    ...
    update_resources: function (resources) {
      ...
      var disabled = record_id < 0 || record_id === null || record_id === undefined;
    ...
    record_message: function (position, size, max_size, record_id) {
      ...
      for (const button_id of buttons) {
        ...
        if (button_id == "print" || button_id == "relate" || button_id == "email") {
          can_be_sensitive |= this.screen.get_buttons().some(function (button) {
      ...
      set_sensitive("switch_", (position || view_type == "form") && has_views);
      ...
      var msg;
      if (size < max_size) {
        msg = name + "@" + Sao.common.humanize(size) + "/" + Sao.common.humanize(max_size);
    ...
    export: function () {
      this.modified_save().then(() => {
        new Sao.Window.Export(this.title.text(), this.screen, this.screen.current_view.get_fields());
      });
    },
    do_export: function (export_) {
      this.modified_save().then(() => {
        ...
        var fields = export_["export_fields."].map((field) => field.name);
        this.screen.model.execute("export_data", [ids, fields, export_.header], this.screen.context).then(function (data) {
          var unparse_obj = {
            data: data,
          };
          ...
          var delimiter = ",";
          if (navigator.platform && navigator.platform.slice(0, 3) == "Win") {
            delimiter = ";";
          }
          ...
          Sao.common.download_file(csv, export_.name + ".csv", { type: "text/csv;charset=utf-8" });
        });
      });
    },
    import: function () {
  ...
  Sao.Tab.Board = Sao.class_(Sao.Tab, {
    ...
    init: function (attributes) {
      ...
      this.view_id = attributes.view_ids.length > 0 ? attributes.view_ids[0] : null;
      ...
      this.view_prm = UIView.execute("view_get", [this.view_id], this.context);
  ...
  Sao.Tab.Wizard = Sao.class_(Sao.Tab, {
    ...
    _close_allowed: function () {
      ...
      if (wizard.state !== wizard.end_state && wizard.end_state in wizard.states) {
        wizard.response(wizard.states[wizard.end_state].attributes);
      }
